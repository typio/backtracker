...
export const createFieldMapping = (fieldNames: string[]): Record<string, DataType | null> => {
...
export const parseCSV = (
  ...
  const parseNumber = (v: string): number => {
    ...
    const base = parseFloat(
      multiplier > 1 ? numString.slice(0, -1) : numString,
    );
  ...
  >((acc, type) => {
    ...
    if (index !== undefined) {
      ...
      acc[type] = rawCol.map(
        type === DataType.date ? parseDate : parseNumber,
      );
    }
    return acc;
  }, {});
  ...
  // Filter invalid rows (must have valid date and close)
  const validRows = rows.filter(
    (row) => !isNaN(row.date) && !isNaN(row.close),
  );
